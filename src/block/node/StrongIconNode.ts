...
const createStrongIconNode: NodeCreator<StrongIconNode | PlainNode> = ([raw], opts) => {
  ...
  const num = numStr.startsWith("*") ? Number.parseInt(numStr.substring(1), 10) : 1;
  return new Array(num).fill({}).map(() => ({ path, pathType, type: "strongIcon", raw }));
};

export const StrongIconNodeParser: NodeParser = createNodeParser(createStrongIconNode, {
  parseOnNested: false,
  parseOnQuoted: true,
